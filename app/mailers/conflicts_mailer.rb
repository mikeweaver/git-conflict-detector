class ConflictsMailer < ActionMailer::Base
  ...
  def self.send_conflict_emails(repository_name,
                                conflicts_newer_than,
                                exclude_branches_if_owned_by_user,
                                hidden_file_list)
    User.subscribed_users.users_with_emails(GlobalSettings.email_filter).each do |user|
      ...
      maybe_send_conflict_email_to_user(
        user,
        repository_name,
        conflicts_newer_than,
        exclude_branches_if_owned_by_user,
  ...
  def maybe_send_conflict_email_to_user(user,
                                        repository_name,
                                        conflicts_newer_than,
                                        exclude_branches_if_owned_by_user,
                                        hidden_file_list)
    ...
    suppressed_owned_branch_ids = exclude_branches_if_owned_by_user.collect(&:id)

    scope = Conflict.from_repository(repository_name).by_user(user).exclude_branches_with_ids(suppressed_branch_ids) \
                    .exclude_non_self_conflicting_authored_branches_with_ids(user, suppressed_owned_branch_ids) \
                    .exclude_conflicts_with_ids(suppressed_conflict_ids)
  ...
  def send_conflict_email_to_user(user, repository_name, new_conflicts, resolved_conflicts, existing_conflicts)
    ...
    mail(
      ...
      subject: "Conflicts Detected in #{repository_name}",
      template_name: 'conflicts_email'
    )
  end
end
