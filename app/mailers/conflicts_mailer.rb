class ConflictsMailer < ActionMailer::Base
  ...
  def self.send_conflict_emails(repo_name, conflicts_newer_than, email_filter_list, email_override, exclude_branches_if_owned_by_user)
    User.users_with_emails(email_filter_list).each do |user|
      if email_override.present?
        user.email = email_override
      end
      maybe_send_conflict_email_to_user(user, repo_name, conflicts_newer_than, exclude_branches_if_owned_by_user)
    end
  end

  def send_conflict_email_to_user(user, repo_name, new_conflicts, resolved_conflicts, existing_conflicts)
  ...
  def self.maybe_send_conflict_email_to_user(user, repo_name, conflicts_newer_than, exclude_branches_if_owned_by_user)
    ...
    unless new_conflicts.blank? && resolved_conflicts.blank?
      ConflictsMailer.send_conflict_email_to_user(
          user,
          repo_name,
          new_conflicts,
          resolved_conflicts,
