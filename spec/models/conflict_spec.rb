...
describe 'Conflict' do
  ...
  before do
    @branches = GitModels::TestHelpers.create_branches(author_name: 'Author Name', count: 3)
  end
  ...
  it 'can be cleared' do
    ...
    Conflict.resolve!(@branches[0], @branches[1], Time.current)
    expect(Conflict.all.size).to eq(1)
  ...
  it 'requires two branches' do
    ...
    expect { create_test_conflict(nil, nil, file_list: ['test/file.rb']) }.to \
      raise_exception(ActiveRecord::RecordInvalid)
  end

  it 'requires a file list array' do
    expect { create_test_conflict(@branches[0], @branches[1], file_list: nil) }.to \
      raise_exception(ActiveRecord::RecordInvalid)
    expect { create_test_conflict(@branches[0], @branches[1], file_list: '') }.to \
      raise_exception(ActiveRecord::RecordInvalid)
  ...
  it 'does not allow conflicts between repositories' do
    branch_in_different_repository = GitModels::TestHelpers.create_branch(repository_name: 'other_repository_name')
    expect { create_test_conflict(@branches[0], branch_in_different_repository) }.to \
      raise_exception(ActiveRecord::RecordInvalid)
  end
  ...
  it 'does not allow duplicate conflicts' do
    ...
    conflict = Conflict.new(
      ...
      status_last_changed_date: Time.current
    )
  ...
  context 'with multiple files' do
    ...
    it 'can return a included filtered list of files by exact match' do
      expect(@conflict.conflicting_files_including(['file1.txt', 'subfolder/file1.txt'])).to \
        eq(['file1.txt', 'subfolder/file1.txt'])
    end
  ...
  context 'with branches from multiple users' do
    before do
      @other_branches = GitModels::TestHelpers.create_branches(author_name: DIFFERENT_NAME, count: 2)
    ...
    it 'can be filtered by branch ids' do
      ...
      conflicts = Conflict.exclude_branches_with_ids(
        [@branches[0].id, @branches[1].id, @other_branches[0], @other_branches[1]]
      )
      expect(conflicts.size).to eq(0)
    ...
    it 'can be filtered by non-self conflicting authored branch ids' do
      ...
      conflicts = Conflict.exclude_non_self_conflicting_authored_branches_with_ids(
        first_user,
      ...
      conflicts = Conflict.exclude_non_self_conflicting_authored_branches_with_ids(
        first_user,
        [@other_branches[0], @other_branches[1]]
      )
      expect(conflicts).to eq([@conflict_1, @conflict_2, @conflict_3])
  ...
  context 'with branches from multiple repositories' do
    it 'returns branches from a repository' do
      ...
      @conflict_2 = create_test_conflict(
        GitModels::TestHelpers.create_branch(repository_name: 'repository_b', name: 'name_a'),
