class Merge < ActiveRecord::Base
  ...
  scope :by_target_user, lambda { |user| joins(:target_branch).where('(branches.author_id = ?)', user.id) }

  scope :created_after, lambda { |after_date| where('merges.created_at >= ?', after_date) }
